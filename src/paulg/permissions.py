...
def confine_to(root: Path | str, read_tools: dict[str, str] = _READ_TOOLS):
    ...
    Allows the Skill tool and read/search tools (``Read``/``Grep``/``Glob``) only
    for paths resolved inside ``root``; denies every other tool, including all
    writes. This guards direct tool calls — note that Claude Code's *native*
    memory feature (enabled separately) writes to its own store and is not routed
